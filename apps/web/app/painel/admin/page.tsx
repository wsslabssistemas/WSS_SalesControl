...
export default async function AdminPage({
  ...
  //
  // ⚠ AS TRÊS LEITURAS QUE CRESCEM SÃO PAGINADAS, e nesta tela o corte tem uma
  // consequência que as outras não têm: **o número que fica menor é o gasto.**
  // A barra do teto global e a coluna "Custo IA" saem daqui. Cortadas em 1.000
  // linhas, o painel afirma que se gastou menos do que se gastou — e um freio
  // de custo lendo menos que o real é o único erro desta tela que não dá para
  // corrigir depois de acontecer.
  //
  // Uma delas passou meses invisível para a trava porque a consulta VIZINHA
  // usava `.maybeSingle()`, e o `paginacao_check` procurava o escape na
  // janela inteira. Hoje ele procura dentro da consulta.
  const [
    { data: tenants },
    cs,
    { data: members },
    ...
    lerTudo<Usage>((de, ate) => {
      // A ordenação e a faixa entram JUNTO com o `.from()`, antes dos filtros
      // condicionais: para o PostgREST a ordem dos encadeados é indiferente, e
      // assim a consulta continua legível como uma coisa só.
      let q = admin.from("usage_ledger").select("tenant_id, cost_cents, tokens_in, tokens_out").order("id").range(de, ate);
      if (janela.de) q = q.gte("occurred_at", janela.de);
      if (janela.ate) q = q.lt("occurred_at", janela.ate);
      return q;
    }, { rotulo: "uso de IA do periodo" }),
    ...
    lerTudo<{ cost_cents: number | null }>((de, ate) => admin
      .from("usage_ledger").select("cost_cents").gte("occurred_at", inicioDoMes)
      .order("id").range(de, ate), { rotulo: "gasto global do mes" }),
  ]);
