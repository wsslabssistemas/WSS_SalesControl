...
export default async function NovaEmpresaPage({
  ...
  return (
    <main style={{ maxWidth: 640, margin: "0 auto", padding: "2rem 1.25rem" }}>
      ...
      {erro && (
        <p className="badge badge-danger mt-16" style={{ width: "100%", justifyContent: "center" }}>
          {erro}
        </p>
      )}
      ...
      <form action={criarEmpresa} className="card mt-24 stack" style={{ gap: 18 }}>
        <label className="stack" style={{ gap: 6 }}>
          ...
          <input name="nome" required placeholder="Como seus clientes te chamam" autoFocus />
        </label>

        <label className="stack" style={{ gap: 6 }}>
          <span className="label" style={{ margin: 0 }}>Cidade</span>
          <input name="cidade" placeholder="Porto Alegre/RS" />
        </label>

        <fieldset style={{ border: "none", padding: 0, margin: 0 }}>
          <legend className="label" style={{ padding: 0, marginBottom: 8 }}>Ramo</legend>
